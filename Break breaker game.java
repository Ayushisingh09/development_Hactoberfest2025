...
class GamePlay extends JPanel implements KeyListener, ActionListener {
    ...
    private int playerX = 310;  // Paddle X Position

    private int ballPosX = 120; // Ball position X
    private int ballPosY = 350; // Ball position Y
    private int ballXdir = -1;  // Ball movement X
    private int ballYdir = -2;  // Ball movement Y
    ...
    public void keyPressed(KeyEvent e) {
        if (e.getKeyCode() == KeyEvent.VK_RIGHT) {
            if (playerX < 600) playerX += 40;
            play = true;
        }
        if (e.getKeyCode() == KeyEvent.VK_LEFT) {
            if (playerX > 10) playerX -= 40;
            play = true;
        }
